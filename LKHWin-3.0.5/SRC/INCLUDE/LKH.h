...
struct Node {
    ...
    char OldPredExcluded, OldSucExcluded;  /* Booleans used for indicating 
                                              ...
                                              has been excluded */
	int OnConvexHull; /* Whether the node lies on the convex hull (If node coordinates are given) */
};
...
/*
Stores result of instersection check.
*/
typedef struct intersectionResult intersectionResult;
struct intersectionResult {
	int NumberOfIntersections;
	Node* S1, * S2; //first nodes of segments in a detected intersection, if there are any intersections.
};
...
Rational RationalIntersectionPointX(Node*, Node*, Node*, Node*);
Rational RationalEvaluatePointOnLine(Rational, Rational, Rational);
Rational RationalSegmentSweepIntersectY(const Node*, Rational);
Rational RfromI(long long int);
Rational RationalSlope(const Node*, const Node*);
int RequalsR(Rational, Rational);
int RstrictlylessR(Rational, Rational);
double RtoDouble(Rational);
...
int *ConvexHull; /*Array of vertices of the convex hull (in case the nodes have a spatial position), in order*/
...
int TSPTW_Makespan;
int UseSweepline; /*Boolean used to toggle whether to use the sweepline algorithm for line segment intersection */
...
intersectionResult compute_intersections(void);
