...
static void Read_CONVEX_HULL_SECTION(void);
...
void ReadProblem()
{
    int i, j, K;
    char *Line, *Keyword;
	//DEBUG -- REMOVE AFTER IMPLEMENTING BENTLEY OTTMAN
	//dict_test();
    ...
    while ((Line = ReadLine(ProblemFile))) {
		...
		if (!strcmp(Keyword, "COMMENT"));
			...
			Read_TYPE();
		else if (!strcmp(Keyword, "CONVEX_HULL_SECTION"))
			Read_CONVEX_HULL_SECTION();
        else
	...
	if (ProblemType == PAREA)
		Penalty = Penalty_PAREA;
    if (BWTSP_B > 0) {
        if (Penalty)
...
int integer_compare(const int* a, const int* b) {
	//int* res;// = NULL;
...
int dict_test() {
	dict* dct = NULL;
	char str[5];
	dct = pr_dict_new((dict_compare_func)integer_compare);
	for (int i = 0; i < 10; i++) {
		sprintf(str, " &d ", 3 * i);
		int* point = malloc(sizeof *point);
		*point = i;
		dict_insert_result res = dict_insert(dct, point);
		if (!res.inserted) { printf("NO insert\n"); }
		*res.datum_ptr = str;
	}
	int j = 5;
	//int* pt1 = &j;
...
static void CheckSpecificationPart()
{
    ...
    if (WeightType != EXPLICIT &&
        ...
        eprintf("Conflicting EDGE_WEIGHT_TYPE and EDGE_WEIGHT_FORMAT");
    if ((ProblemType == ATSP || ProblemType == SOP || ProblemType == PAREA) &&
...
static void Read_CONVEX_HULL_SECTION()
{
	int CH_size, i,j;
	Node* N;
	if (!fscanf(ProblemFile, "CONVEX_HULL_SIZE : %d", &CH_size))
		eprintf("CONVEX_HULL_SECTION : Missing CH size");
	assert(ConvexHull = (int*)calloc(CH_size, sizeof(int)));
	//Set values of CH
	for (i = 1; i <= Dimension; i++) {
		NodeSet[i].OnConvexHull = 0;
	}
	for (i = 0; i < CH_size; i++) {
		if (!fscanint(ProblemFile, &ConvexHull[i]))
			eprintf("CONVEX_HULL_SECTION : Missing node");
		NodeSet[ConvexHull[i]].OnConvexHull = 1;
	}
	/*Node* N;
	//printff("first %d\n", ConvexHull[0]);
	for (i = 0; i < CH_size; i++) {
		N = &NodeSet[ConvexHull[i]];
		printff("\npos: %d; Node #%d: coord X %f; Y %f;\n", i, N->Id, N->X, N->Y);
	}*/
}
...
static void Read_TYPE()
{
	...
	Asymmetric =
		...
		ProblemType == VRPB ||
		ProblemType == VRPBTW ||
		ProblemType == VRPPD || ProblemType == PAREA;
}
